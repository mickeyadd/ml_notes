...
def credit_ratings_plot(ratings_df):
    """
    Plots:
    1. Distribution of credit ratings with Spectral gradient.
    2. Distribution of sectors colored by average rating.

    Assumes 'Rating_Code' and 'Sector' columns exist in ratings_df.
    """
    ...
    # Ensure Rating_Code exists
    if 'Rating_Code' not in ratings_df.columns:
        ratings_df['Rating_Code'] = ratings_df['Rating'].cat.codes

    # Compute average rating per sector
    avg_rating_per_sector = ratings_df.groupby('Sector', observed=True)['Rating_Code'].mean().sort_values()

    # Create color palette based on average rating
    palette = sns.color_palette("Spectral", n_colors=len(avg_rating_per_sector))
    sector_colors = {sector: palette[i] for i, sector in enumerate(avg_rating_per_sector.index)}
    ...
    # Left plot: Ratings distribution with gradient palette
    rating_palette = sns.color_palette("Spectral", n_colors=len(ratings_df['Rating'].cat.categories))
    sns.countplot(
        ax=axes[0],
        x='Rating',
        data=ratings_df,
        order=ratings_df['Rating'].cat.categories,
        palette=rating_palette,
        hue='Rating',
        legend=False
    )
    axes[0].set_title('Distribution of Credit Ratings', color='white')
    axes[0].set_xlabel('Rating', color='white')
    axes[0].set_ylabel('Count', color='white')
    axes[0].tick_params(colors='white')

    # Right plot: Sectors colored by average rating
    sns.countplot(
        ax=axes[1],
        y='Sector',
        data=ratings_df,
        order=avg_rating_per_sector.index,
        palette=sector_colors,
        hue='Sector',
        legend=False
    )
    axes[1].set_title('Distribution of Sectors by Average Rating', color='white')
    axes[1].set_xlabel('Count', color='white')
    axes[1].set_ylabel('Sector', color='white')
